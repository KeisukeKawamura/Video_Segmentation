...
const std::string SEGMENTATION_FOLDER = "G:/Segmentation_test/";
...
int main()
{
	...
	cv::VideoCapture cap = cv::VideoCapture(VIDEO_FOLDER + "6.avi");
	cv::VideoWriter wrt;
	wrt.open(SEGMENTATION_FOLDER + "6.avi", cap.get(CV_CAP_PROP_FOURCC), cap.get(CV_CAP_PROP_FPS), cv::Size(cap.get(CV_CAP_PROP_FRAME_WIDTH), cap.get(CV_CAP_PROP_FRAME_HEIGHT)));
